...
# Exercise: What happens if you try to convert say to numeric
a_word <- "hello"

# Exercise: What happends if you convert a logical to numeric? character?
im_true <- TRUE
im_false <- FALSE
```
...
# Exercise: Get all values that are larger than square root of 27
# Hint: R has a function called sqrt() to compute the sqrt of a number
...
A `data.frame` is used to store and analyze rectangular data
  ...
  - For the rest of this tutorial, we will use the `tibble` form produced by `readr`
      - This form is already available to use as a variable called `dbts_df`
...
# Exerercise: Get all columns except 2 and rows 1 - 10
```
...
# Exercise: Return all even rows from 1 - 10 and the "age" and "insulin columns"
...
Tidyverse a set of R packages intended to make data analysis easier
...
# Get counts and propotion for the "pos" group using []?
...
```{r cont-plot2, exercise = TRUE, exercise.lines=8}
...
Data collection is never perfect and you are bound to have bad or missing values
...
# Exercise: What are the row numbers (positions) 
...
```{r corr2, exercise = TRUE, exercise.lines=5}
...
# Proportion table of age vs diabetes
...
# A challenge to try later, can you use observed and expected to compute the chi-sq?
...
`lm()` is used to fit linear regression models in R
  ...
  - `summary()` can be used to view results of regression
